...
| Deep-read | none / skimmed / full — which tier of the coverage contract it got. In FULL-READ mode every `source` row is `full` by definition; in SELECTIVE mode it's earned per path |
| Note | One-line role hypothesis or question |
...
- **Sweep is never budgeted; deep reads follow the sweep's policy.** `analyze.py sweep` prints the verdict and it is binding:
  - **FULL-READ mode** (small repos: ≤ 100 source files and ≤ 50k source LOC, defaults overridable): every `source` path is deep-read `full` by definition — the sweep proved it affordable. Layer 2 grep-outlining is replaced by reading each source file; reads are seam-first (entry points and handoffs lead the attention; file interiors get classification). Generated/vendored stay swept-only; manifests, migrations, and design inputs are read per their own rules.
  - **SELECTIVE mode** (large repos): deep reads go where the evidence value is highest — seams on the critical path, high fan-in, load-bearing config — and the `Deep-read` column records `none`/`skimmed`/`full` as each path earns it.
...
- The sweep's deep-read policy is satisfied: in FULL-READ mode, every source file has been read in full; in SELECTIVE mode, every deep-read claim in the ledger reflects an actual read.
